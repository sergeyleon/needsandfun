...
private function _getGoods($page = 1, $categories = false, $category = false)
    ...
    public function import($values)  // Baltic IT adds
    {
       ...
       move_uploaded_file($_FILES["excel"]["tmp_name"], $_SERVER['DOCUMENT_ROOT']."/app/uploads/excel/".$_FILES["excel"]["name"]);
      ...
      if (!file_exists($_SERVER['DOCUMENT_ROOT']."/app/uploads/excel/".$_FILES["excel"]["name"])) {
      	exit("Please run 05featuredemo.php first.\n");
      }
      
      $objReader = \PHPExcel_IOFactory::createReader('Excel2007');
      $objPHPExcel = $objReader->load($_SERVER['DOCUMENT_ROOT']."/app/uploads/excel/".$_FILES["excel"]["name"]);
